...
def generate_population(label_cities, population_size):
    ...
    while len(unique_solutions) < population_size:  # Generar 500 soluciones únicas
        ...
        if not any(np.array_equal(solution, s) for s in unique_solutions):
            unique_solutions.append(solution.tolist())
...
def evaluate_solutions(solution):
    evaluated_solutions = []
...
parent1 = [171615, 41, 7, 89, 45, 53, 22, 59, 51, 19, 23, 33, 46, 56, 78, 93, 90, 9, 6, 44, 48, 15, 2, 49, 94, 71, 39, 65, 73, 64, 84, 21, 100, 43, 18, 67, 58, 50, 69, 66, 85, 12, 95, 77, 42, 97, 26, 1, 92, 99, 16, 34, 25, 72, 3, 37, 76, 74, 47, 11, 55, 31, 4, 83, 8, 60, 87, 82, 10, 80, 29, 35, 14, 32, 57, 62, 63, 54, 96, 79, 24, 30, 88, 17, 28, 68, 75, 98, 5, 91, 36, 52, 70, 40, 86, 20, 27, 38, 61, 81, 13]
...
population = generate_population(label_cities, 100)
#best = 0
...
for i in range(generations):
    ...
    best_population = tournament(evaluate_solutions, 2)
    new_generation = []
